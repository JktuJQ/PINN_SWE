...
from swe_constants import (X_MIN, X_MAX, Y_MIN, Y_MAX, T_MIN, T_MAX, DEVICE)
from swe_sampler import (sample_interior, sample_initial, sample_boundary_x,
...
class PINN(nn.Module):
    ...
    def __init__(self, hidden=C.HIDDEN, depth=C.DEPTH, fourier=C.USE_FOURIER):
        ...
        in_dim = 3
        if fourier:
            B = torch.randn(3, C.FOURIER_FEATURES) * C.FOURIER_SCALE
        ...
        self.register_buffer("in_lo", torch.tensor([[X_MIN, Y_MIN, T_MIN]]))
        self.register_buffer("in_hi", torch.tensor([[X_MAX, Y_MAX, T_MAX]]))
    ...
    # Функция потерь
    # ------------------------------------------------------------------
    def _pde_loss(self, x, y, t, nu, t_max, causal=True):
        R = residual(self, x, y, t, nu=nu)
        r2 = (R ** 2).mean(dim=1)
    ...
    def compute_losses(self, batch, nu, t_max):
        x_f, y_f, t_f = batch["interior"]
        L_pde = self._pde_loss(x_f, y_f, t_f, nu, t_max)

        x_i, y_i, t_i = batch["ic"]
        h_p, u_p, v_p = self(x_i, y_i, t_i)
        h_t_, u_t_, v_t_ = initial_condition(x_i, y_i)
        L_ic = (((h_p - h_t_) ** 2).mean()
                + ((u_p - u_t_) ** 2).mean()
                + ((v_p - v_t_) ** 2).mean())

        x_b, y_b, t_b, h_far = batch["bc_x"]
        h_bx, u_bx, v_bx = self(x_b, y_b, t_b)
        # точное дальнее поле: волны сюда не доходят
        L_bc = (((h_bx - h_far) ** 2).mean()
                + (u_bx ** 2).mean()
                + (v_bx ** 2).mean())

        x_y, y_y, t_y = batch["bc_y"]
        _, _, v_by = self(x_y, y_y, t_y)
        # только непротекание; u и h на этих стенках свободны
    ...
    def pinn_train(self, adam_epochs=C.ADAM_EPOCHS, lbfgs_steps=C.LBFGS_STEPS,
        ...
        for epoch in pbar:
            ...
            if C.RAR_EVERY and epoch > 0 and epoch % C.RAR_EVERY == 0:
                rar = refine_points(self, residual, nu, t_max=t_max)

            x_f, y_f, t_f = sample_interior(t_max=t_max)
            if rar is not None:
        ...
        if lbfgs_steps > 0:
            if verbose:
                print("L-BFGS дожим...")
            nu = C.VISC_END
            x_f, y_f, t_f = sample_interior(n=C.N_F * 2, t_max=T_MAX)
            batch = {
                "interior": (x_f, y_f, t_f),
        ...
        torch.save(self.state_dict(), C.MODEL_PATH)
        return self, history


def load_trained(path=C.MODEL_PATH):
