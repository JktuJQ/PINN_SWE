...
def shock_sharpness(
    ...
    This is a general metric that works for any problem with a shock wave.
    It finds the shock location by locating the maximum gradient of h(x),
    then measures the width of the front between 10% and 90% of the jump.

    Args:
        x: 1D array of x-coordinates (sorted).
        h: 1D array of water depth values.
        t: Time at which the profile is evaluated (unused, kept for API consistency).
        problem: Optional problem object (unused in this general implementation).

    Returns:
        Width of the shock front (x_90 - x_10). Returns NaN if the front
        cannot be detected (e.g., not enough points near the shock).
    """
    if len(x) < 5:
        return float("nan")

    dh_dx = np.gradient(h, x)
    shock_idx = np.argmax(np.abs(dh_dx))
    x_shock = x[shock_idx]

    window_size = 2.0
    window = (x > x_shock - window_size) & (x < x_shock + window_size)
    if window.sum() < 5:
        return float("nan")
    ...
    def crossing(level: float) -> float | None:
        """Find x where h(x) crosses the given level (linear interpolation)."""
        d = hs - level
